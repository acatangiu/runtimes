...
impl<T: Config> crate::types::AhMigrationCheck for AccountsMigrator<T> {
	...
	fn pre_check(_: Self::RcPrePayload) -> Self::AhPrePayload {
		...
		println!(
			"🤡 AH Pre Check: checking_balance {:?}, ah_total {:?}",
	...
	fn post_check(rc_total_issuance_before: Self::RcPrePayload, _: Self::AhPrePayload) {
		...
		let check_account = T::CheckingAccount::get();
		let ah_checking_balance = <T as Config>::Currency::total_balance(&check_account);
		println!(
			"🤡 AH Post Check: rc_total_before {:?}, ah_total_after {:?}, ah_check_after {:?}",
			rc_total_issuance_before, ah_total_issuance, ah_checking_balance
		);
